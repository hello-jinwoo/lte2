    ...
    def forward(self, x, scale_factor):
        ...
        if self.args.no_upsampling:
            ...
            if type(self.args.upsample_mode) == str:
                res = F.interpolate(res, target_scale=scale_factor, mode=self.args.upsample_mode)
            elif type(self.args.upsample_mode) == list:
                tmp_res = F.interpolate(res, target_scale=scale_factor, mode=self.args.upsample_mode[0])
                for m in self.args.upsample_mode[1:]:
                    tmp_res = tmp_res + F.interpolate(res, target_scale=scale_factor, mode=m)
                tmp_res = tmp_res / len(self.args.upsample_mode)
                res = tmp_res
            else:
                res = F.interpolate(res, target_scale=scale_factor, mode='bicubic')
