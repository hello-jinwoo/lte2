...
v8 -> v9 : replace pos_enc with area_enc
"""
...
class EDSR(nn.Module):
    ...
    def forward(self, x, scale_factor=None, size=None, mode='test'): 
        ...
        # add area_enc
        area_enc = self.area_enc_sinu_2d(d_model=padded_res.shape[1], height=padded_res.shape[2], width=padded_res.shape[3]).to(x.device)
        padded_res = padded_res + area_enc
        ...
        res = x_mhsa[:, :, :h, :w] # cut out the padded part # (B, D, h, w)
