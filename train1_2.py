...
from test import batched_predict_with_sf
...
def eval(model, data_name, save_dir, scale_factor=4):
    ...
    for gt_path in gt_images:
        ...
        with torch.no_grad():
            ...
            output = batched_predict_with_sf(model, 
                                             ((input_tensor - 0.5) / 0.5), 
                                             hr_coord.cuda(), 
                                             cell_factor*cell.cuda(), 
                                             bsize=30000, 
...
def train(train_loader, model, optimizer, epoch, config):
    ...
    for batch in tqdm(train_loader, leave=False, desc=descript, mininterval=2):
        ...
        pred = model(inp, hr_coord.cuda(), cell.cuda(), scale_factor=sf)
